...
// customIndexes defines composite non-unique indexes that cannot be expressed via
// GORM tags: the columns live in an embedded struct shared between a resource and
// its draft table, so a tag there would emit the same index name on both tables.
// Format: table_name -> index definition (without the trailing comma)
var customIndexes = map[string]string{
	"redirects": "INDEX `idx_redirects_ns_proj_type` (`namespace_code`, `project_code`, `type`)",
	"pages":     "INDEX `idx_pages_ns_proj_type` (`namespace_code`, `project_code`, `type`)",
}
...
func main() {
	...
	stmts = addIndexes(stmts, uniqueIndexes)

	// Add composite indexes that GORM tags cannot express
	stmts = addIndexes(stmts, customIndexes)
...
// addIndexes injects index definitions into CREATE TABLE statements
func addIndexes(sql string, indexes map[string]string) string {
	for table, indexDef := range indexes {
