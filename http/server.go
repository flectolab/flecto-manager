...
func CreateServerHTTP(ctx *context.Context) (*echo.Echo, error) {
	...
	roleService := service.NewRoleService(ctx.Validator, roleRepo, userRepo, db)
	...
	projectGroup.POST("/agents", project.PostAgent(permissionChecker, agentService))
	projectGroup.PATCH(fmt.Sprintf("/agents/:%s/hit", key.NameKey), project.PatchAgentHit(permissionChecker, agentService))
