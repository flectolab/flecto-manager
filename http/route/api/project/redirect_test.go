...
func TestGetRedirectsCursor(t *testing.T) {
	...
	redirects := func(count int, firstID int64) []model.Redirect {
		items := make([]model.Redirect, 0, count)
		for i := 0; i < count; i++ {
			items = append(items, model.Redirect{
				ID:            firstID + int64(i),
				NamespaceCode: "ns1",
				ProjectCode:   "proj1",
				Redirect:      &commonTypes.Redirect{Source: "/old", Target: "/new"},
	...
	t.Run("a full page hands out a cursor", func(t *testing.T) {
		...
		mockRedirectService.EXPECT().
			...
			Return(redirects(2, 10), int64(7), nil)

		c, rec := newContext(echo.New(), "/?limit=2")
		require.NoError(t, GetRedirects(permissionChecker, mockRedirectService)(c))

		var body commonTypes.RedirectList
	...
	t.Run("a short page ends the walk", func(t *testing.T) {
		...
		mockRedirectService.EXPECT().
			FindByProjectPublished(gomock.Any(), "ns1", "proj1", gomock.Any(), nil).
			Return(redirects(1, 10), int64(1), nil)
	...
	t.Run("a cursor page reports the total and offset it carries", func(t *testing.T) {
		...
		mockRedirectService.EXPECT().
			FindByProjectPublished(gomock.Any(), "ns1", "proj1", gomock.Any(), gomock.Any()).
			DoAndReturn(func(_ context.Context, _, _ string, _ *commonTypes.PaginationInput, afterID *int64) ([]model.Redirect, int64, error) {
				gotAfterID = afterID
				// The service does not count in cursor mode, so it reports no total.
				return redirects(2, 12), 0, nil
			})
	...
	t.Run("a malformed cursor is a client error", func(t *testing.T) {
		...
		err := GetRedirects(permissionChecker, mockRedirectService)(c)
