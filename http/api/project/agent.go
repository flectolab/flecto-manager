...
func PostAgent(permissionChecker *auth.PermissionChecker, agentService service.AgentService) func(echo.Context) error {
	return func(c echo.Context) error {
		...
		namespaceCode := c.Param(key.NamespaceCodeKey)
		projectCode := c.Param(key.ProjectCodeKey)
		...
		if !permissionChecker.CanResource(userCtx.SubjectPermissions, namespaceCode, projectCode, model.ActionWrite) {
