...
func (s *agentService) CountByProjectAndStatus(ctx context.Context, namespaceCode, projectCode string, status commonTypes.AgentStatus, updatedAfter time.Time) (int64, error) {
	return s.repo.CountByProjectAndStatus(ctx, namespaceCode, projectCode, status, updatedAfter)
}
