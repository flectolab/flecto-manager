...
func setupProjectServiceTest(t *testing.T) *projectServiceTestDeps {
	...
	svc := NewProjectService(validator.New(), mockProjRepo, mockPageRepo, mockRedirectDraftRepo, mockPageDraftRepo, defaultProjectCfg, nil)
...
func TestProjectService_Publish(t *testing.T) {
	...
	t.Run("success with redirect drafts create/update", func(t *testing.T) {
		...
		svc := NewProjectService(validator.New(), projRepo, pageRepo, redirectDraftRepo, pageDraftRepo, defaultProjectCfg, db)
