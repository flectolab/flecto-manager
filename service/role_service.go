...
type RoleService interface {
	...
	SearchPaginate(ctx context.Context, pagination *types.PaginationInput, query *gorm.DB) (*model.RoleList, error)
...
type roleService struct {
	...
	adminRepo    repository.AdminPermissionRepository
	db           *gorm.DB
}
...
func (s *roleService) SearchPaginate(ctx context.Context, pagination *types.PaginationInput, query *gorm.DB) (*model.RoleList, error) {
...
func (s *roleService) GetPermissionsByRoleCode(ctx context.Context, code string) (*model.SubjectPermissions, error) {
	...
	resources, err := s.resourceRepo.FindByRoleID(ctx, role.ID)
	if err != nil {
		return nil, err
	}

	admin, err := s.adminRepo.FindByRoleID(ctx, role.ID)
	if err != nil {
		return nil, err
	}
...
func (s *roleService) GetPermissionsByUsername(ctx context.Context, username string) (*model.SubjectPermissions, error) {
	...
	resources, err := s.resourceRepo.FindByRoleIDs(ctx, roleIDs)
	if err != nil {
		return nil, err
	}

	admin, err := s.adminRepo.FindByRoleIDs(ctx, roleIDs)
	if err != nil {
		return nil, err
	}
...
func deduplicateResourcePermissions(perms []model.ResourcePermission) []model.ResourcePermission {
	...
	for _, p := range perms {
		key := p.Namespace + "|" + p.Project + "|" + string(p.Action)
