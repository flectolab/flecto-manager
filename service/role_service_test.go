...
type roleServiceMocks struct {
	...
	resourceRepo *mockFlectoRepository.MockResourcePermissionRepository
	adminRepo    *mockFlectoRepository.MockAdminPermissionRepository
}
...
func setupRoleServiceTest(t *testing.T) (*roleServiceMocks, RoleService) {
	...
	mocks := &roleServiceMocks{
		...
		resourceRepo: mockFlectoRepository.NewMockResourcePermissionRepository(ctrl),
		adminRepo:    mockFlectoRepository.NewMockAdminPermissionRepository(ctrl),
	}
	svc := NewRoleService(validator.New(), mocks.roleRepo, mocks.userRepo, mocks.resourceRepo, mocks.adminRepo, nil)
	return mocks, svc
}
...
func TestRoleService_GetPermissionsByRoleCode(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		ctx := context.Background()
		role := &model.Role{ID: 1, Code: "admin"}
		...
		mocks.resourceRepo.EXPECT().
			FindByRoleID(ctx, int64(1)).
			Return(resources, nil)

		mocks.adminRepo.EXPECT().
			FindByRoleID(ctx, int64(1)).
			Return(admin, nil)
	...
	t.Run("error from resource permissions", func(t *testing.T) {
		mocks, svc := setupRoleServiceTest(t)
		defer mocks.ctrl.Finish()

		ctx := context.Background()
		role := &model.Role{ID: 1, Code: "admin"}
		expectedErr := errors.New("resource fetch error")
	...
	t.Run("error from admin permissions", func(t *testing.T) {
		mocks, svc := setupRoleServiceTest(t)
		defer mocks.ctrl.Finish()
		...
		role := &model.Role{ID: 1, Code: "admin"}
		resources := []model.ResourcePermission{}
		expectedErr := errors.New("admin fetch error")
...
func TestRoleService_GetPermissionsByUsername(t *testing.T) {
	...
	t.Run("error from FindByRoleIDs (resources)", func(t *testing.T) {
		mocks, svc := setupRoleServiceTest(t)
		defer mocks.ctrl.Finish()

		ctx := context.Background()
		user := &model.User{ID: 1, Username: "testuser"}
		roles := []model.Role{{ID: 1, Code: "role1"}}
		expectedErr := errors.New("resource fetch error")
	...
	t.Run("error from FindByRoleIDs (admin)", func(t *testing.T) {
		mocks, svc := setupRoleServiceTest(t)
		defer mocks.ctrl.Finish()
...
func TestDeduplicateResourcePermissions(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "no duplicates",
			input: []model.ResourcePermission{
				{Namespace: "ns1", Project: "p1", Action: model.ActionRead},
				{Namespace: "ns2", Project: "p2", Action: model.ActionWrite},
			},
			expected: 2,
		},
		{
...
func setupRoleServiceIntegrationTest(t *testing.T) (*gorm.DB, RoleService) {
	...
	adminRepo := repository.NewAdminPermissionRepository(db)

	svc := NewRoleService(validator.New(), roleRepo, userRepo, resourceRepo, adminRepo, db)
	return db, svc
}
...
func TestRoleService_UpdateRolePermissions_Integration(t *testing.T) {
	t.Run("success - replace all permissions", func(t *testing.T) {
		...
		newPermissions := &model.SubjectPermissions{
			Resources: []model.ResourcePermission{
				...
				{Namespace: "new-ns2", Project: "new-proj2", Action: model.ActionDelete},
			},
			Admin: []model.AdminPermission{
				{Section: model.AdminSectionRoles, Action: model.ActionWrite},
			},
	...
	t.Run("does not affect other roles permissions", func(t *testing.T) {
		...
		newPermissions := &model.SubjectPermissions{
			Resources: []model.ResourcePermission{
				{Namespace: "updated-ns", Project: "updated-proj", Action: model.ActionDelete},
			},
			Admin: []model.AdminPermission{},
		}
