...
func TestServiceJWT_GenerateAccessToken(t *testing.T) {
	...
	token, expiresAt, err := service.GenerateAccessToken(user, AuthTypeBasic, nil, nil)
...
func TestServiceJWT_GenerateRefreshToken(t *testing.T) {
	...
	token, expiresAt, err := service.GenerateRefreshToken(user, AuthTypeBasic, nil, nil)
...
func TestServiceJWT_GenerateTokenPair(t *testing.T) {
	...
	tokenPair, err := service.GenerateTokenPair(user, AuthTypeBasic, nil, nil)
...
func TestServiceJWT_generateToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			token, expiresAt, err := service.generateToken(user, AuthTypeBasic, tt.tokenType, permissions, []string{"role"}, tt.ttl)
...
func TestServiceJWT_parseToken(t *testing.T) {
	...
	t.Run("parses valid token", func(t *testing.T) {
		token, _, err := service.GenerateAccessToken(user, AuthTypeBasic, nil, nil)
		assert.NoError(t, err)
	...
	t.Run("fails on token signed with wrong secret", func(t *testing.T) {
		...
		token, _, err := otherService.GenerateAccessToken(user, AuthTypeBasic, nil, nil)
		assert.NoError(t, err)
	...
	t.Run("fails on expired token", func(t *testing.T) {
		...
		token, _, err := service.generateToken(user, AuthTypeBasic, TokenTypeAccess, nil, nil, -1*time.Hour)
		assert.NoError(t, err)
...
func TestServiceJWT_TokenExpiration(t *testing.T) {
	...
	t.Run("access token has correct expiration", func(t *testing.T) {
		_, expiresAt, err := service.GenerateAccessToken(user, AuthTypeBasic, nil, nil)
		assert.NoError(t, err)
	...
	t.Run("refresh token has correct expiration", func(t *testing.T) {
		_, expiresAt, err := service.GenerateRefreshToken(user, AuthTypeBasic, nil, nil)
		assert.NoError(t, err)
