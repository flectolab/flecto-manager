...
func (c *PermissionChecker) CanResourceForUsername(ctx context.Context, username, namespace, project string, action model.ActionType) (bool, error) {
	permissions, err := c.roleService.GetPermissionsByUsername(ctx, username)
	if err != nil {
		return false, err
	}
	return c.CanResource(permissions, namespace, project, action), nil
}
...
func (c *PermissionChecker) CanResourceForRoleCode(ctx context.Context, roleCode, namespace, project string, action model.ActionType) (bool, error) {
...
func (c *PermissionChecker) MustCanResourceForUsername(ctx context.Context, username, namespace, project string, action model.ActionType) bool {
	result, _ := c.CanResourceForUsername(ctx, username, namespace, project, action)
	return result
}
...
func (c *PermissionChecker) MustCanResourceForRoleCode(ctx context.Context, roleCode, namespace, project string, action model.ActionType) bool {
	result, _ := c.CanResourceForRoleCode(ctx, roleCode, namespace, project, action)
	return result
}
...
func (c *PermissionChecker) CanResource(permissions *model.SubjectPermissions, namespace, project string, action model.ActionType) bool {
	for _, p := range permissions.Resources {
		if c.matchResource(p, namespace, project, action) {
			return true
		}
	}
	return false
...
func (c *PermissionChecker) matchResource(p model.ResourcePermission, namespace, project string, action model.ActionType) bool {
	...
	return namespaceMatch && projectMatch && actionMatch
}
