...
func initDatabase(db *gorm.DB) error {
	...
	db.Exec(`ALTER TABLE redirects MODIFY COLUMN source VARCHAR(600)`)
	db.Exec(`ALTER TABLE redirects MODIFY COLUMN status VARCHAR(50)`)
	db.Exec(`ALTER TABLE redirects MODIFY COLUMN type VARCHAR(50)`)
	db.Exec(`ALTER TABLE redirect_drafts MODIFY COLUMN new_source VARCHAR(600)`)
	db.Exec(`ALTER TABLE redirect_drafts MODIFY COLUMN new_status VARCHAR(50)`)
	db.Exec(`ALTER TABLE redirect_drafts MODIFY COLUMN new_type VARCHAR(50)`)
	db.Exec(`ALTER TABLE pages MODIFY COLUMN path VARCHAR(600)`)
	db.Exec(`ALTER TABLE pages MODIFY COLUMN type VARCHAR(50)`)
	db.Exec(`ALTER TABLE pages MODIFY COLUMN content_type VARCHAR(50)`)
	db.Exec(`ALTER TABLE page_drafts MODIFY COLUMN new_path VARCHAR(600)`)
	db.Exec(`ALTER TABLE page_drafts MODIFY COLUMN new_type VARCHAR(50)`)
	db.Exec(`ALTER TABLE page_drafts MODIFY COLUMN new_content_type VARCHAR(50)`)
	db.Exec(`ALTER TABLE agents MODIFY COLUMN name VARCHAR(100)`)
	db.Exec(`ALTER TABLE agents MODIFY COLUMN status VARCHAR(50)`)
	db.Exec(`ALTER TABLE agents MODIFY COLUMN type VARCHAR(50)`)

	db.Exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_redirects_source_unique USING BTREE ON redirects(namespace_code, project_code, source)`)
	db.Exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_redirect_drafts_source_unique USING BTREE ON redirect_drafts(namespace_code, project_code, new_source)`)
	db.Exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_pages_path_unique USING BTREE ON pages(namespace_code, project_code, path)`)
	db.Exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_page_drafts_path_unique USING BTREE ON page_drafts(namespace_code, project_code, new_path)`)
	db.Exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_agents_namespace_project_name USING BTREE ON agents(namespace_code, project_code, name)`)
	return nil
}
