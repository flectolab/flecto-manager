...
type AgentRepository interface {
	...
	CountByProjectAndStatus(ctx context.Context, namespaceCode, projectCode string, status commonTypes.AgentStatus, updatedAfter time.Time) (int64, error)
...
func (r *agentRepository) Upsert(ctx context.Context, agent *model.Agent) error {
	...
	if agent.Status != "" {
		...
		updates["error"] = agent.Error
		updates["load_duration"] = agent.LoadDuration
	}
...
func (r *agentRepository) CountByProjectAndStatus(ctx context.Context, namespaceCode, projectCode string, status commonTypes.AgentStatus, updatedAfter time.Time) (int64, error) {
	var count int64
	err := r.db.WithContext(ctx).
		Model(&model.Agent{}).
		Where(fmt.Sprintf("%s = ? AND %s = ? AND status = ? AND updated_at >= ?", model.ColumnNamespaceCode, model.ColumnProjectCode), namespaceCode, projectCode, status, updatedAfter).
		Count(&count).Error
	if err != nil {
		return 0, err
	}
