...
func TestAgentRepository_Upsert(t *testing.T) {
	...
	t.Run("update existing agent with status", func(t *testing.T) {
		db := setupAgentTestDB(t)
		createTestAgentNamespace(t, db, "test-ns", "Test Namespace")
		createTestAgentProject(t, db, "test-ns", "test-proj", "Test Project")
		repo := NewAgentRepository(db)
	...
	t.Run("update existing agent without status only updates updated_at", func(t *testing.T) {
		db := setupAgentTestDB(t)
		createTestAgentNamespace(t, db, "test-ns", "Test Namespace")
		createTestAgentProject(t, db, "test-ns", "test-proj", "Test Project")
		repo := NewAgentRepository(db)
		...
		db.First(&originalAgent, originalID)
		originalUpdatedAt := originalAgent.UpdatedAt
		...
		db.First(&found, originalID)
		assert.Equal(t, commonTypes.AgentStatusSuccess, found.Status)
		assert.Equal(t, 100*time.Millisecond, found.LoadDuration)
		assert.True(t, found.UpdatedAt.After(originalUpdatedAt))
	})

	t.Run("update does not change type", func(t *testing.T) {
		db := setupAgentTestDB(t)
		createTestAgentNamespace(t, db, "test-ns", "Test Namespace")
		createTestAgentProject(t, db, "test-ns", "test-proj", "Test Project")
		repo := NewAgentRepository(db)
...
func TestAgentRepository_CountByProjectAndStatus(t *testing.T) {
	...
	t.Run("only counts agents updated after threshold", func(t *testing.T) {
		db := setupAgentTestDB(t)
		createTestAgentNamespace(t, db, "test-ns", "Test Namespace")
		createTestAgentProject(t, db, "test-ns", "test-proj", "Test Project")
		repo := NewAgentRepository(db)
