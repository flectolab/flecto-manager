...
export function UserForm() {
  ...
  const canDelete = canAdminResource(AdminSection.Users, Action.Delete)
  ...
  useEffect(() => {
    if (userData?.user) {
      ...
      setUserRoles(userData.user.roles.filter((r): r is string => r !== null))
      setResourcePermissions(userData.user.permissions.resources.map(p => ({
        ...p,
        type: p.type as ResourcePermission['type'],
      })))
      setAdminPermissions(userData.user.permissions.admin.map(p => ({
        ...p,
        type: p.type as AdminPermission['type'],
      })))
      setIsModified(false)
  ...
  const availableRoles = rolesData?.roles
    ...
    .filter(name => name.toLowerCase().includes(roleInput.toLowerCase())) || []
  ...
  const handleAddResourcePermission = () => {
    setResourcePermissions([
      ...resourcePermissions,
      { type: 'user', namespace: '*', project: '*', action: 'read' }
    ])
    setPermissionsModified(true)
  }
